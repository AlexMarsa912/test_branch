...
class PersonInfo:
    ...
    def new_salary(self):
        popular_letters = sorted([("".join(self.subdivision)).count(letter) for letter in
                                  set("".join(self.subdivision))], reverse=True)
        return 1337 * self.age * (sum(popular_letters[:3]))
